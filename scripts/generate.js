...
function renderQuestion(title, q, a, choices, id, explanation) {
    let inputHtml = '';
...
${choices.map((c, i) => `      <label class="choice-label"><input type="radio" name="q${id}" value="${c}" class="user-answer-radio"> ${c}</label>`).join('\n')}
    ...
    const explHtml = explanation ? `<div class="explanation-text">解説：${explanation}</div>` : '';

    // Markdownパーサーが誤認しないよう、空行を排除した形式で出力する
  ...
  <div class="question-block" data-correct="${a}">
    <p class="question-text">${q.replace(/\n/g, '<br>')}</p>
${inputHtml}
    <span class="judgment-mark" id="mark-q${id}"></span>
    <div class="answer-hidden">正解：<strong>${a.replace(/\|/g, ' または ')}</strong>${explHtml}</div>
  </div>
  <div class="step-nav">
...
quizData.forEach((day) => {
    ...
    const q1 = renderQuestion(q1Title, q1Text, day.a1, day.choices1, 1, day.expl1);
    const q2 = renderQuestion("買い物計算", day.q2 || day.買い物計算, day.a2, day.choices2, 2, day.expl2);
    const q3 = renderQuestion("難読漢字", day.q3 || day.難読漢字, day.a3, day.choices3, 3, day.expl3);
    const q4 = renderQuestion("言葉パズル", day.q4 || day.言葉パズル, day.a4, day.choices4, 4, day.expl4);
    const q5 = renderQuestion("ひらめきパズル", day.q5, day.a5, day.choices5, 5, day.expl5);
    const q6 = renderQuestion("記憶力テスト", day.q6, day.a6, day.choices6, 6, day.expl6);
    const q7 = renderQuestion("間違い探し", day.q7, day.a7, day.choices7, 7, day.expl7);
